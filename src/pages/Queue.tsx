...
export default function Queue() {
    ...
    const fetchPosts = async () => {
        ...
        const res = await fetch(`${HOST}/api/media_item?limit=${pageSize}&offset=${offset}`, {
            method: "GET",
            headers: {
    ...
    const filteredPosts = useMemo(() => {
        return posts.filter((post) => {
            ...
            return post.title.toLowerCase().includes(searchLower) || post.body.toLowerCase().includes(searchLower);
        });
    }, [searchQuery, posts]);
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>{"Comments (relevance / all)"}</TableHead>
                                        ...
                                        <TableRow key={post.id} onClick={() => nav(`/media_items/${post.id}`)} className="cursor-pointer hover:bg-muted/30">
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-1">
                                                    {post.comments_cnt ? (
                                                        <>
                                                            <span className="text-sm font-medium">{post.relevant_comments_cnt}</span>
                                                            <span className="text-xs text-muted-foreground">{` / ${post.comments_cnt}`}</span>
                                                        </>
                                                    ) : (
                                            ...
                                            <TableCell>
                                                <span className="text-xs text-muted-foreground">{formatRelativeTime(post.created_at_platform)}</span>
                                            </TableCell>
                                            <TableCell>
